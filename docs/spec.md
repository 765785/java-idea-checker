...
两个粘贴入口的时序必须分离：第 2 步“本机”框只接收 JavaCheck.bat 的 JSON、控制台原文或 result.txt；先展示 A 区结论与第 3/4 步修复复检，随后才进入 B 区。第 6 步“官网”框位于官网登录回传块中，只有点击“我已登录好”才会展开。两框在解析前识别交叉内容：官网关键词进入本机框、或 JSON 字段进入官网框，均立即停止、说明粘错位置并提供滚动高亮到正确框的按钮。两框分别使用不同边框、标题、占位符和已/未粘贴摘要；本机摘要包含采集时间和脱敏用户名，官网摘要只包含脱敏域名/邮箱证据。
...
| N59 | 本机检测结果框固定在第 2 步；官网回传框仅在 A 区结果后、第 6 步登录确认后显示 | J10、T |
...
| J10 | 本机结果框在第 2 步；官网回传框在 A 区结果之后，初始隐藏 |
