...
规则优先级为：N54–N58 / Q29–Q30 优先于 N38–N42 / Q27–Q28；后者优先于 N32–N37 / Q23–Q26；未被明确作废的早期规则继续有效。本轮不改变 ZIP 唯一下载入口、四脚本结构、免下载 A/B、离线 HTML、杀软折叠区、四步流程和 PowerShell 版本 guard。
...
3. 回网页 A3 Ctrl+V 粘贴 JSON，或拖入 result.txt；浏览器仅将导入内容作为纯文本数据处理。
...
最终屏幕提示固定为：“请回到浏览器，在 A3 的框里按 Ctrl+V，然后点解析。”失败时仅显示中文原因与下一步；完整异常写到脱敏日志或 errors，不能向学生直接输出英文堆栈。
...
B 区页面从上到下固定为四块：机器证据（自动）→ IDEA 当前账户人证 → 官网登录回传 → 最终动作。学校域名白名单的唯一真源是 app.js 中 EDU_DOMAIN_WHITELIST；工具包生成时注入 JavaCheck.ps1 的 EduDomains 数组，脚本中不保留第二份硬编码，也不接收个人邮箱。顶部 B 区摘要按“官网回传 → IDEA 人证 → 机器证据”选择最高优先级来源。

机器证据扫描新版、旧版与 Toolbox，跳过缓存/日志/重解析点，限制扩展名、单文件 5MB、总文件 3000。每个允许文本文件同时匹配白名单域名字面量、完整邮箱和 XML 账户关键行；账户/许可证文件与 options/other.xml 中的其他域名可触发机器侧明显不符，无关文件中的其他邮箱仅作参考。单人自用个人电脑是假定前提：账户类文件命中本校域名就是高可信机器证据，账户类文件命中 QQ、163 等其他域名时明确提示可能登录错账号。多人共用机器的结果仅作参考。cmdkey 只记录 JetBrains/IntelliJ/Toolbox 条目名，凭据库仅记录 credential/kdbx/keychain/secure/store 类文件名，绝不读取密码或凭据内容。条目、片段、日志和报告共用统一脱敏。

官网回传必须先登录再复制：用户主动打开 JetBrains 官方登录页，以教育邮箱完成登录，然后点“我已登录好，下一步”才展示订阅管理页链接和复制框。密码只输入给 JetBrains；页面没有 password 输入框、不索取密码、不保存或上传密码。回传文本低于 200 字符、只有 Sign in/Log in/登录/注册入口且没有邮箱或学校域名、或完全没有邮箱和学校域名时，状态为 NOT_LOGGED_IN，提示回到第一步，禁止将其解析成“未找到教育包”。回传页面全文只在当前内存中判断，导出报告只记录已登录/未登录/未进行和脱敏证据。

两个粘贴入口的时序必须分离且都永久可见：A3“本机”框只接收 JavaCheck.bat 的 JSON、控制台原文或 result.txt；B3“官网”框用于登录后的订阅页文本，未登录文本按登录态规则提示。两框分别使用不同边框、图标、标题、占位符和已/未粘贴摘要；本机摘要包含采集时间和脱敏用户名，官网摘要只包含脱敏域名/邮箱证据。

### N62–N67：粘贴、解锁与发布缓存

- N62：A3 只做检测结果的白名单正向识别：顶层可解析 JSON 有 `schemaVersion`、或 `meta`/`env`/`exec`/`javaProbe`/`idea` 任意两个字段、或 `JAVA_HOME=`、或 `java version`/`openjdk version`/`java 版本`、或以 `{` 开头且可提取平衡 JSON 块，命中任何一项即解析。不得用 `Sign in`、`Licenses`、`Educational`、登录等网页词排除；未命中仅给通用的“没能识别出这是检测结果”提示。
- N63：B3 仅在“完整 JSON 顶层有 `schemaVersion`”或同时含 `schemaVersion` 与 `JAVA_HOME` 时，提示检测结果应去 A3；普通含 `java` 或单独含 `JAVA_HOME` 的网页文本仍按回传文本解析。
- N64：schema 1 或其他已填写的未知版本统一提示重新下载最新版 ZIP，不得混同为粘错；未带版本号但命中其他检测特征的完整结果继续解析，并在 A 区结论标注“未识别到版本号”。
- N65：所有滚动定位使用 `focusOrExplain`：目标不存在时显示“这一步还没准备好，请先完成上一步”，并定位到当前可操作的上一步；存在时才滚动、聚焦和高亮。
- N66：A 区成功解析后才显示 A4；失败原因写在 A3 框正下方，并保留“先完成上面的检测并粘贴结果，之后这里会出现修复选项。”。A 区没有 FAIL 时显示“环境正常，无需修复”。
- N67：页面可见位置标注 `v1.3.0` 和更新日期；样式和脚本 URL 都带同一版本查询串。每次发布必须同步递增页面版本号、`assets/*?v=` 查询串和离线版/Release 产物；版本不一致时显示可关闭的 Ctrl+F5 提示。

### N68–N73：A / B 模块解耦

- N68：页面固定为 `section-a` 的 A 部分（A1 下载 ZIP、A2 运行检测脚本、A3 粘贴检测结果、A4 修复并复检）和 `section-b` 的 B 部分（B1 机器证据、B2 人证邮箱、B3 官网登录后复制文本、B4 回 IDEA 点 Activate）。页首有两个锚点导航；不得再使用跨模块连续步骤编号。
- N69：B 部分永久渲染，不以 A 区是否解析成功为条件。没有 A 结果时 B1 明确显示“这部分需要 A 部分的检测结果。你可以先做 B2、B3、B4。”，而不是隐藏 B 部分。
- N70：`paste-detection` 与 `paste-licenses` 永久存在、可见、可输入且不 disabled。B3 的登录确认只是提示；粘贴文本会直接按登录态规则校验。
- N71：A3 标明“本机”且使用橙色边框，B3 标明“官网”且使用蓝色边框；两框各自保留填充状态和脱敏摘要。
- N72：定位仅依赖 `section-a`、`section-b`、`paste-detection`、`paste-licenses` 四个固定 id。
- N73：所有纠错跳转统一通过 `focusOrExplain`；目标不存在时显示“这一步还没准备好，请先完成上一步”并滚动到可操作位置，目标存在时滚动、聚焦、高亮。

最终四档为：登录后的官网回传命中白名单域名最高可信；其次是合格 IDEA 人证或机器命中白名单域名；凭据管理器条目或凭据库文件是中可信；只有目录、无证据或 UNKNOWN 是需人工确认；官网登录回传、人证或账户/许可证主文件存在其他域名时明显不符。人证仅在 IDEA 已安装且存在配置目录时参与升档；官网回传优先于人证，人证优先于机器。未安装或未启动 IDEA 是 MANUAL，不影响 A 区。任何档位均不得说认证成功或教育包已激活。
...
验收编号 S01–S35 是机器场景；F01–F07 是第四轮专项；G01–G18 为第五轮自动/人工验收；H01–H12 为第六轮追加验收；I01–I12 为 B 区定稿验收；J01–J15 为官网回传与双粘贴框验收。T 表示自动契约，V 表示真实 Windows、VM 或人工验收。第二轮没有独立原文；N1–N5、P1–P6 仅登记对话明确引用的含义，不补造缺失规则。
...
| N54 | 单人个人机上账户类本校证据高可信，账户类其他域名明确提示账号不符 | J01–J02、T |
| N55 | 官网回传必须先登录、确认后才展示复制订阅页文本的输入框 | J03–J05、T |
| N56 | 未登录/过短/无邮箱文本只提示回登录，不判没有教育包 | J04、T |
| N57 | 密码只由学生在官方页面输入；本页无 password 字段或索取密码行为 | J06、T |
| N58 | B 区固定机器→人证→官网回传→最终动作；优先级官网→人证→机器 | J05、J07、T |
| Q29 | 官网三步显示进度、登录后粘贴提示和完成收尾文案 | J07–J08、T |
| Q30 | 导出报告记录官网回传登录态，且不含页面全文或完整邮箱 | J09、T |
| N59 | 已由 N68–N73 取代：A3 与 B3 是独立且永久可见的粘贴框 | N68–N73、T |
| N60 | 两个粘贴框在解析前识别对方内容，拦截后说明该去哪个步骤 | J11–J12、T |
| N61 | 本机与官网框使用不同标题、配色、占位符与独立粘贴状态 | J13、T |
| Q31 | 粘错提示提供“带我去正确的框”，滚动、聚焦并高亮目标 | J14、T |
| Q32 | 两个框粘对后分别显示脱敏采集摘要与域名/邮箱摘要 | J15、T |
...
| J01 | 个人机账户类文件命中 dnui.edu.cn 为高可信机器证据 |
| J02 | 账户类文件命中 QQ/163 等其他域名为明显不符，文案明确提示账号可能错误 |
| J03 | 未点“我已登录好”时订阅页复制框保持隐藏 |
| J04 | Sign in/登录入口、过短或无邮箱文本为 NOT_LOGGED_IN，不判未找到教育包 |
| J05 | 登录后含 dnui.edu.cn 与订阅信息的文本为最高优先级高可信 |
| J06 | 页面没有 password 输入框，也不索取密码 |
| J07 | B 区四块顺序与证据优先级固定 |
| J08 | 官网三步进度、粘贴提示和完成收尾存在 |
| J09 | 报告含官网登录回传登录态，且没有完整邮箱或页面全文 |
| J10 | 已由 N68–N73 取代：A3、B3 的两个框始终可见，B 区不依赖 A 区 |
| J11 | 官网文本粘到本机结果框时在 JSON 解析前拦截，不生成 A/B 结论 |
| J12 | 检测 JSON 粘到官网框时在登录态解析前拦截，不生成认证结论 |
| J13 | 两框标题、边框、图标、placeholder 和“已/未粘贴”状态可明显区分 |
| J14 | 拦截提示可一键滚动、聚焦、高亮正确框 |
| J15 | 本机框显示采集时间和脱敏用户；官网框显示脱敏域名/邮箱证据 |
