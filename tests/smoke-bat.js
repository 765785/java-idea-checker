...
const dir=path.resolve(__dirname,'..','.cache','中文 检测 smoke');fs.mkdirSync(dir,{recursive:true});const bat=path.join(dir,'JavaCheck.bat');const ps1=path.join(dir,'JavaCheck.ps1');fs.writeFileSync(bat,t.buildCheckerBat(),'ascii');fs.writeFileSync(ps1,t.buildCheckerPs1(a.EDU_DOMAIN_WHITELIST),'utf8');
...
fs.writeFileSync(path.join(dir,'console.txt'),r.stdout+'\n'+r.stderr,'utf8');assert.equal(r.status,0,r.stderr||String(r.error));const d=a.parseResult(fs.readFileSync(path.join(dir,'result.txt'),'utf8'));assert.equal(d.schemaVersion,2);console.log(JSON.stringify({report:path.join(dir,'result.txt'),status:a.analyze(d).status,errors:d.errors},null,2));
