...
3. 普通双击 **JavaCheck.bat**，**不要以管理员身份运行检测**。黑色窗口结束时会提示你回到浏览器，在第 2 步的橙色“本机检测结果”框中按 Ctrl+V 再点“解析”。也可以拖入同文件夹的 result.txt；这里不要粘 JetBrains 官网文本。
4. 只有网页 A 区显示 FAIL 时，才在**同一个解压文件夹**普通双击 **JavaRepair.bat**。备份目录、repair.log 和 result.txt 都写在脚本所在目录。完成后关闭当前黑色窗口，重新双击 JavaCheck.bat，再粘贴新的结果复检。教育认证的蓝色“官网回传”框在页面更下方，必须完成官网登录引导后才会出现，不能与第 2 步框混用。
...
学校教育邮箱域名固定为 “dnui.edu.cn”，邮箱名前缀每人不同，工具不会猜测、校验或补全前缀。本工具假定你使用的是自己的个人电脑，一人一机；多人共用电脑上的扫描结果只能作参考。B 区按“机器证据 → IDEA 人证 → 官网登录回传”给出四档可信度，官网回传优先级最高：

- **高可信**：登录后的 JetBrains 官网订阅页回传了 @dnui.edu.cn，或 IDEA 当前账户人证为 @dnui.edu.cn，或个人电脑的账户/配置证据命中本校域名。
- **中可信**：发现 JetBrains 凭据管理器条目，或发现 IDEA 配置目录中的疑似凭据库文件，但未读到本校域名。
- **需人工确认**：仅存在配置目录、没有自动证据，或采集发生权限/占用异常。没找到证据 ≠ 认证失败。
- **明显不符**：官网登录回传、人证，或账户/许可证主文件中的证据是 QQ、163 等其他域名。请换回本校教育邮箱登录；这通常表示你可能登录错账号了。

脚本会扫描 IntelliJ、旧版配置和 Toolbox；只读取允许的文本文件，凭据库仅记录文件名，cmdkey 仅记录 JetBrains 条目名，绝不读取密码。无关文件中的 QQ/163 等邮箱只是参考，不会单独判定登录错账号。官网登录回传优先于 IDEA 人证，IDEA 人证优先于机器扫描。

### 官网登录回传：先登录，再复制

1. 在结果页 B 区第 3 块点“打开 JetBrains 账户登录页”，用自己的 `@dnui.edu.cn` 教育邮箱登录。密码只输入给 JetBrains 官方网站；本网页不会看到、保存或要求你填写密码。
2. 点“我已登录好，下一步”，打开订阅管理页。确认右上角显示的是自己的教育邮箱后，按 Ctrl+A、Ctrl+C 复制页面文本，再回到网页粘贴。
3. 页面会拦下过短、只有 Sign in/登录入口、或完全没有邮箱/本校域名的文本，并提示你先回去登录；不会把这种未登录页面误判为“没有教育包”。

两个粘贴框有明确分工：第 2 步橙色“本机”框只接收 JavaCheck 运行结果；第 6 步蓝色“官网”框只接收登录后的订阅页面文本。把网页文本粘进本机框，或把 JSON 粘进官网框时，网页会在解析前拦住，并提供“带我去正确的框”按钮。粘对后，本机框会显示采集时间与脱敏用户名，官网框只显示脱敏域名/邮箱证据，便于你自己核对。

官网登录回传文本仅在当前页面内存中用于判断登录态和域名，导出报告只记录“已登录 / 未登录 / 未进行”和脱敏邮箱证据，不保存页面全文。
...
| 文件 | SHA-256 | 版本 | 日期 |
| --- | --- | --- | --- |
| Release: JavaIDEA-checker.zip | `f5aea985893aa1c348223d210f2753bfa9ca6200cfe4b90fb9e28787d99fd318` | v1.2.0 | 2026-09-25 |
| Release: JavaIDEA-checker-offline.html | `39923536cfdd74c345a2f24b15b9a496e93a094bc7c9513496b7305e6e39e806` | v1.2.0 | 2026-09-25 |
